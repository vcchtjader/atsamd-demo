...
mod app {
    ...
    fn init(cx: init::Context) -> (Shared, Local, init::Monotonics()) {
        ...
        writeln!(
            ...
            "Column 1: Is generated signature identical to a reference signature?",
        )
        .unwrap();
        writeln!(
            u as &mut dyn Write<_, Error = _>,
            "Column 2: Is a signature valid according to PUKCC",
        )
        .unwrap();
        writeln!(
            u as &mut dyn Write<_, Error = _>,
            "Column 3: Is a broken signature invalid according to PUKCC",
        )
        .unwrap();
        writeln!(
            u as &mut dyn Write<_, Error = _>,
            "Test vector: {} samples",
            K_SIGNATURE_PAIRS.len()
        )
        .unwrap();
        for (i, (k, reference_signature)) in K_SIGNATURE_PAIRS.iter().enumerate() {
            ...
            writeln!(
                u as &mut dyn Write<_, Error = _>,
                "{:>2}: {:<5} | {:<5} | {:<5}",
                i, are_signatures_same, is_signature_valid, is_broken_signature_invalid,
            )
            .unwrap();
        }
