...
use atsamd_hal::{
    clock::v2::{
        gclk, xosc::*, xosc32k::*, Dpll, Gclk1Div, GclkDiv, GclkIn, GclkOut, Pclk, Tokens,
    },
    gpio::v2::Pins,
    time::U32Ext,
};
...
mod app {
    ...
    use atsamd_hal::clock::v2::DpllXoscPreDivider;
    ...
    fn init(cx: init::Context) -> (init::LateResources, init::Monotonics()) {
        ...
        //let pclk_dpll0 = Dpll::from_pclk(tokens.sources.dpll0, pclk_dpll0);
        //let dpll0 = pclk_dpll0.set_loop_div(50, 0).enable();

        // Configure DPLL0 to 100 MHz fed from Xosc0
        let (dpll0, _xosc0) =
            Dpll::from_xosc(tokens.sources.dpll0, xosc0, DpllXoscPreDivider::Div(1));

        // Use the predivider, 8 MHz / 4 * 50 = 100 MHz

        let dpll0 = dpll0
            .set_source_div(DpllXoscPreDivider::Div(1))
            .set_loop_div(50, 0)
            .enable();
        ...
        let gclk_in3 = GclkIn::enable(tokens.sources.gclk_io.gclk_in3, pins.pb17, 2.mhz());
        ...
        let (_gclk_out6, _gclk6) =
            GclkOut::enable(tokens.sources.gclk_io.gclk_out6, pins.pb20, gclk6, false);
        ...
        //let (dpll1, xosc32k) = Dpll::from_xosc32k(tokens.sources.dpll1, xosc32k);
